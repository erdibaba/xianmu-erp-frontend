<template>
  <div class="mod-erp-product">
    ...
    <el-table :data="dataList" border stripe v-loading="dataListLoading" @selection-change="selectionChangeHandle">
      ...
      <el-table-column prop="originCountry" label="原产地" width="100" align="center" header-align="center"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="150" align="center" header-align="center">
        <template slot-scope="scope">
          <el-button v-if="isAuth('erp:product:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          <el-button v-if="isAuth('erp:product:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/erp/product/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行删除操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/erp/product/delete'),
            method: 'post',
